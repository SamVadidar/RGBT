...
from Fusion.utils.general import xyxy2xywh, xywh2xyxy
from Fusion.utils.torch_utils import torch_distributed_zero_first
...
def create_dataloader9(path, imgsz, batch_size, stride, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, img_format='.jpg'):
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache
    with torch_distributed_zero_first(rank):
...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, auto_size=32, img_format = '.jpg'):
        ...
        images = [x for x in files if x.split('.')[-1].lower() in img_format]
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            self.img_files = sorted([x.replace('/', os.sep) for x in f if os.path.splitext(x)[-1].lower() in img_format])
        ...
        try:
            ...
            for p in path if isinstance(path, list) else [path]:
                ...
                if p.is_dir():  # dir
                    ...
                    raise Exception('%s does not exist' % p)
            self.img_files = sorted([x.replace('/', os.sep) for x in f if x.split('.')[-1].lower() in img_format])
            assert self.img_files, 'No images found'
        except Exception as e:
            raise Exception('Error loading data from %s: %s\nSee %s' % (path, e, help_url))
...
def recursive_dataset2bmp(dataset='path/dataset_bmp', img_format = '.jpg'):  # from utils.datasets import *; recursive_dataset2bmp()
    # Converts dataset to bmp (for faster training)
    formats = [x.lower() for x in img_format] + [x.upper() for x in img_format]

...
from Fusion.utils.general import bbox_iou
from Fusion.utils.torch_utils import is_parallel
...
def compute_loss(p, targets, hyp, dict):  # predictions, targets, model
    ...
    lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device) # losses
    tcls, tbox, indices, anchors = build_targets(p, targets, hyp, dict)  # targets
    h = hyp  # hyperparameters

    # Define criteria
    BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([h['cls_pw']])).to(device) # BinaryCrossEntropy
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions    
        ...
        if n:
            ...
            iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, CIoU=True)  # giou(prediction, target)
            lbox += (1.0 - iou).mean()  # giou loss

            # Objectness
            tobj[b, a, gj, gi] = (1.0 - dict['gr']) + dict['gr'] * iou.detach().clamp(0).type(tobj.dtype)  # giou ratio
...
def build_targets(p, targets, hyp, dict):
    ...
    # Multi GPU is Not Yet Supported in this Script
    ...
    for i, anchors in enumerate(torch.from_numpy(np.array(dict['anchors_g'])).reshape(3,3,2).to(dict['device'])):#model.module.yolo_layers if multi_gpu else model.yolo_layers):
        ...
        a, t, offsets = [], targets * gain, 0 # targets * gain = targets adapted to the feature map size
